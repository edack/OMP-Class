...
class DirToUSSDirHandler extends ZosFilesBase_handler_1.ZosFilesBaseHandler {
    processWithSession(commandParameters, session) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (attributesFile) {
                response = yield this.uploadWithAttributesFile(attributesFile, response, session, inputDir, commandParameters, status);
            }
            else {
                const filesMap = this.buildFilesMap(commandParameters);
                if (commandParameters.arguments.recursive) {
                    response = yield zos_files_for_zowe_sdk_1.Upload.dirToUSSDirRecursive(session, inputDir, commandParameters.arguments.USSDir, {
                    ...
                    response = yield zos_files_for_zowe_sdk_1.Upload.dirToUSSDir(session, inputDir, commandParameters.arguments.USSDir, {
                        binary: commandParameters.arguments.binary,
                        filesMap,
                        maxConcurrentRequests: commandParameters.arguments.maxConcurrentRequests,
                        task: status,
    ...
    findAttributesFile(commandParameters, inputDir) {
        let attributesFile;
        if (commandParameters.arguments.attributes) {
            if (!fs.existsSync(commandParameters.arguments.attributes)) {
                throw new imperative_1.ImperativeError({ msg: imperative_1.TextUtils.formatMessage(zos_files_for_zowe_sdk_1.ZosFilesMessages.attributesFileNotFound.message, { file: commandParameters.arguments.attributes }) });
            }
    ...
    uploadWithAttributesFile(attributesFile, response, session, inputDir, commandParameters, status) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                throw new imperative_1.ImperativeError({ msg: imperative_1.TextUtils.formatMessage(zos_files_for_zowe_sdk_1.ZosFilesMessages.errorReadingAttributesFile.message, { file: attributesFile, message: err.message }) });
            }
            let attributes;
            try {
                attributes = new zos_files_for_zowe_sdk_1.ZosFilesAttributes(attributesFileContents, inputDir);
            }
            catch (err) {
                throw new imperative_1.ImperativeError({ msg: imperative_1.TextUtils.formatMessage(zos_files_for_zowe_sdk_1.ZosFilesMessages.errorParsingAttributesFile.message, { file: attributesFile, message: err.message }) });
            }
            if (commandParameters.arguments.recursive) {
                response = yield zos_files_for_zowe_sdk_1.Upload.dirToUSSDirRecursive(session, inputDir, commandParameters.arguments.USSDir, {
                    attributes,
