...
class Upload {
    ...
    static bufferToDataSet(session, fileBuffer, dataSetName, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const uploadRequest = yield core_for_zowe_sdk_1.ZosmfRestClient.putExpectFullResponse(session, requestOptions);
     ...
     */
    static bufferToUssFile(session, ussname, buffer, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const headers = this.generateHeadersBasedOnOptions(options, "buffer");
            return core_for_zowe_sdk_1.ZosmfRestClient.putExpectString(session, ZosFiles_constants_1.ZosFilesConstants.RESOURCE + parameters, headers, buffer);
        });
    }
    /**
     * Upload content to USS file
     ...
     * @returns {Promise<object>}
     */
    static bufferToUSSFile(session, ussname, buffer, binary = false, localEncoding, etag, returnEtag, responseTimeout) {
        return __awaiter(this, void 0, void 0, function* () {
     ...
     */
    static streamToUssFile(session, ussname, uploadStream, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            imperative_1.ImperativeExpect.toNotBeNullOrUndefined(ussname, ZosFiles_messages_1.ZosFilesMessages.missingUSSFileName.message);
            ...
            const parameters = ZosFiles_constants_1.ZosFilesConstants.RES_USS_FILES + "/" + ussname;
            const reqHeaders = this.generateHeadersBasedOnOptions(options, "stream");
            // Options to use the stream to write a file
            const restOptions = {
                resource: ZosFiles_constants_1.ZosFilesConstants.RESOURCE + parameters,
                reqHeaders,
            ...
            // If requestor needs etag, add header + get "response" back
            if (options.returnEtag) {
                restOptions.dataToReturn = [ZosmfRestClientProperties_1.CLIENT_PROPERTY.response];
            }
            const uploadRequest = yield core_for_zowe_sdk_1.ZosmfRestClient.putExpectFullResponse(session, restOptions);
            // By default, apiResponse is empty when uploading
            const apiResponse = {};
            // Return Etag in apiResponse, if requested
            if (options.returnEtag) {
     ...
     * @returns {Promise<object>}
     */
    static streamToUSSFile(session, ussname, uploadStream, binary = false, localEncoding, task, etag) {
        return __awaiter(this, void 0, void 0, function* () {
     ...
     * @returns {Promise<IZosFilesResponse>} - A response indicating the outcome
     */
    static fileToUssFile(session, inputFile, ussname, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            imperative_1.ImperativeExpect.toNotBeNullOrUndefined(inputFile, ZosFiles_messages_1.ZosFilesMessages.missingInputFile.message);
            imperative_1.ImperativeExpect.toNotBeNullOrUndefined(ussname, ZosFiles_messages_1.ZosFilesMessages.missingUSSFileName.message);
            ...
            const request = yield this.streamToUssFile(session, ussname, uploadStream, options);
            result = {
                success: true,
                from: inputFile,
                to: ussname
     ...
     * @param etag
     */
    static fileToUSSFile(session, inputFile, ussname, binary = false, localEncoding, task, etag, returnEtag) {
        return __awaiter(this, void 0, void 0, function* () {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (maxConcurrentRequests === 0) {
                    ...
                    yield core_for_zowe_sdk_1.asyncPool(maxConcurrentRequests, filesArray, createFileUploadPromise);
                }
                const result = {
                    success: true,
                    from: inputDirectory,
    ...
    static isDirectoryExist(session, ussname) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const response = yield core_for_zowe_sdk_1.ZosmfRestClient.getExpectJSON(session, ZosFiles_constants_1.ZosFilesConstants.RESOURCE + parameters);
                if (response.items) {
                    return true;
                }
            }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (directoriesArray.length > 0) {
                ...
                if (maxConcurrentRequests === 0) {
                    ...
                    yield core_for_zowe_sdk_1.asyncPool(maxConcurrentRequests, directoriesArray, createDirUploadPromise);
                }
            }
            ...
            if (filesArray.length > 0) {
                ...
                if (maxConcurrentRequests === 0) {
                    ...
                    yield core_for_zowe_sdk_1.asyncPool(maxConcurrentRequests, filesArray, createUploadPromise);
                }
            }
            const result = {
                success: true,
    ...
    static uploadFile(localPath, ussPath, session, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (options.attributes) {
                ...
                yield this.fileToUssFile(session, localPath, ussPath, { binary: tempBinary });
            }
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (attributes.fileShouldBeUploaded(localPath)) {
                ...
                if (binary) {
                    ...
                    yield this.fileToUssFile(session, localPath, ussPath, { binary, localEncoding: attributes.getLocalEncoding(localPath) });
                }
     ...
     */
    static generateHeadersBasedOnOptions(options, context) {
        const reqHeaders = [];
        switch (context) {
            case "stream":
            case "buffer":
                if (options.binary) {
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.OCTET_STREAM);
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_BINARY);
                }
                else if (options.localEncoding) {
                    reqHeaders.push({ "Content-Type": options.localEncoding });
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_TEXT);
                }
                    ...
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.TEXT_PLAIN);
                }
                ...
                break;
            default:
                const headers = ZosFilesUtils_1.ZosFilesUtils.generateHeadersBasedOnOptions(options);
                if (headers.length === 0) {
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_TEXT);
                }
                else if (headers.length === 1 &&
                    core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_RESPONSE_TIMEOUT in headers[0]) {
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_TEXT);
        ...
        if (options.recall) {
            switch (options.recall.toLowerCase()) {
                case "wait":
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_MIGRATED_RECALL_WAIT);
                    break;
                case "nowait":
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_MIGRATED_RECALL_NO_WAIT);
                    break;
                case "error":
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_MIGRATED_RECALL_ERROR);
                    break;
                default:
                    reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_MIGRATED_RECALL_NO_WAIT);
                    break;
        ...
        if (options.returnEtag) {
            reqHeaders.push(core_for_zowe_sdk_1.ZosmfHeaders.X_IBM_RETURN_ETAG);
        }
        return reqHeaders;
    }
}
