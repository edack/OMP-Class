...
class Imperative {
    ...
    static getResolvedCmdTree(config) {
        return DefinitionTreeResolver_1.DefinitionTreeResolver.resolve(config.rootCommandDescription || "", config.productDisplayName, path_1.dirname(utilities_1.ImperativeConfig.instance.callerLocation), this.log, config.definitions, config.commandModuleGlobs, config.baseProfile != null);
    }
    /**
    ...
    static getPreparedCmdTree(resolvedCmdTree, baseProfile) {
        ...
        preparedCmdTree = cmd_1.CommandPreparer.prepare(preparedCmdTree, baseProfile);
    ...
    static addAutoGeneratedCommands(rootCommand) {
        ...
        if ((loadedConfig.autoGenerateProfileCommands == null || loadedConfig.autoGenerateProfileCommands) &&
            ...
            rootCommand.children.push(CompleteProfilesGroupBuilder_1.CompleteProfilesGroupBuilder.getProfileGroup(allProfiles, this.log));
        }
        const authConfigs = {};
            ...
            loadedConfig.profiles.forEach((profile) => {
                if (profile.authConfig != null) {
                    for (const requiredOption of ["host", "port", "user", "password", "tokenType", "tokenValue"]) {
                        expect_1.ImperativeExpect.toNotBeNullOrUndefined(profile.schema.properties[requiredOption], `Profile of type ${profile.type} with authConfig property must have ${requiredOption} option defined`);
                    }
