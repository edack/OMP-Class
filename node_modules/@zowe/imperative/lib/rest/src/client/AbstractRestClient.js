...
class AbstractRestClient {
    ...
    request(options) {
        return new Promise((resolve, reject) => {
            ...
            this.mReject = reject;
            expect_1.ImperativeExpect.toBeDefinedAndNonBlank(options.resource, "resource");
            expect_1.ImperativeExpect.toBeDefinedAndNonBlank(options.request, "request");
            expect_1.ImperativeExpect.toBeEqual(options.requestStream != null && options.writeData != null, false, "You cannot specify both writeData and writeStream");
    ...
    requestHandler(res) {
        ...
        if (this.requestSuccess) {
            ...
            if (this.response.headers != null) {
                ...
                let encoding;
                if (Headers_1.Headers.CONTENT_ENCODING in this.response.headers) {
                    encoding = this.response.headers[Headers_1.Headers.CONTENT_ENCODING];
                }
                if (Headers_1.Headers.CONTENT_ENCODING.toLowerCase() in this.response.headers) {
                    encoding = this.response.headers[Headers_1.Headers.CONTENT_ENCODING.toLowerCase()];
                }
                if (typeof encoding === "string" && Headers_1.Headers.CONTENT_ENCODING_TYPES.find((x) => x === encoding)) {
                    this.log.debug("Content encoding of response is: " + encoding);
                    if (this.mDecode) {
