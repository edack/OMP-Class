...
exports.WebHelpGenerator = void 0;
const fs = require("fs");
...
class WebHelpGenerator {
    ...
    buildHelp(cmdResponse) {
        ...
        const distDir = path.join(__dirname, "../../../../web-help/dist");
        ...
        dirsToCopy.forEach((dir) => {
            ...
            fs.readdirSync(dir)
                ...
                .forEach((filename) => fsExtra.copySync(path.join(dir, filename), path.join(destDir, filename)));
        });
        ...
        let cmdTreeHtml = "<ul>";
        this.treeNodes[0].children.forEach((node) => {
...
`;
     ...
     */
    appendToSinglePageHtml(definition, rootCommandName, fullCommandName, htmlContent) {
    ...
    genCommandHelpPage(definition, fullCommandName, docsDir, parentNode) {
        ...
        htmlContent = htmlContent.replace(/<code>\$\s*(.*?)<\/code>/g, `<code>$1</code> <button class="btn-copy no-print" data-balloon-pos="right" data-clipboard-text="$1">Copy</button>`);
