...
exports.CommandProfiles = void 0;
...
class CommandProfiles {
    ...
    constructor(map, metaMap) {
         ...
         * @memberof CommandProfiles
         */
        this.mMetaMap = new Map();
        // Simple validation of input parameters
        const err = "Command Profiles Internal Error:";
        expect_1.ImperativeExpect.toNotBeNullOrUndefined(map, `${err} No map was supplied.`);
        expect_1.ImperativeExpect.toBeEqual(map instanceof Map, true, `${err} The "map" supplied is not an instance of a map.`);
        // Ensure the correctness of each map entry
        map.forEach((value, key) => {
            expect_1.ImperativeExpect.toBeAnArray(value, `${err} The "profiles" supplied for type "${key}" is NOT an array.`);
            expect_1.ImperativeExpect.toBeEqual((value.length > 0), true, `${err} No profiles supplied for type "${key}".`);
        });
        this.mMap = map;
        if (metaMap) {
     ...
     * @memberof CommandProfiles
     */
    getMeta(type, failNotFound = true, name = "") {
        let profile;
     ...
     * @memberof CommandProfiles
     */
    get(type, failNotFound = true, name = "") {
        let profile;
        // If a profile is returned for the type, then we'll check if a profile of a specific name was requseted
        // if not, just return the first profile found (first loaded)
...
exports.CommandProfiles = CommandProfiles;
//# sourceMappingURL=CommandProfiles.js.map